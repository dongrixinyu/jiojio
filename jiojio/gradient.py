...
def get_grad_SGD_minibatch(node_grad: np.ndarray, edge_grad: np.ndarray,
    ...
    total_without_edge_num = 0
    for x in X:
        # start_time = time.time()
        error, feature_id_set, node_num, with_edge_num, without_edge_num = get_grad_CRF(
            node_grad, edge_grad, model, x)

        total_num += node_num
        total_with_edge_num += with_edge_num
        total_without_edge_num += without_edge_num
    ...
    edge_grad /= len(X)
    print('total: {}, with_edge_num: {}, without_edge_num: {}.'.format(
        total_num, total_with_edge_num, total_without_edge_num))

    bi_ratio_grad = (total_without_edge_num - total_with_edge_num) / total_num
...
def get_grad_CRF(node_grad: np.ndarray, edge_grad: np.ndarray,
    ...
    sum_edge_masked = get_masked_beliefs(belief_masked, masked_Y, model.bi_ratio)
