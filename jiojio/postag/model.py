...
class Model:
    ...
    def load(cls, model_dir):
        ...
        if os.path.exists(model_path):
            ...
            model.n_transition_feature = model.n_tag * (model.n_feature + model.n_tag)
            model.w = w
            assert model.w.shape[0] == model.n_transition_feature
            return model

        print("WARNING: weights.npz does not exist, try loading using old format", file=sys.stderr)
    ...
    def new(cls, model, copy_weight=True):
        ...
        new_model.n_feature = (new_model.w.shape[0] // new_model.n_tag - new_model.n_tag)
