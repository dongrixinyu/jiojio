...
def _get_node_features_idx(idx, nodes, feature_to_idx):
    flist = []
    i = idx
    length = len(nodes)
    j = 0
...
class FeatureExtractor:
    ...
    def get_node_features_idx(self, idx, nodes):
        return _get_node_features_idx(idx, nodes, self.feature_to_idx)
    ...
    def convert_feature_file_to_idx_file(self, feature_file, feature_idx_file, tag_idx_file):
        ...
        with open(feature_idx_file, "w", encoding="utf8") as f_writer, \
            ...
            for line in lines:
                ...
                splits = line.split(" ")
                feature_idx = [self.feature_to_idx[feat] for feat in splits[:-1] if feat in self.feature_to_idx]
        ...
        with open(text_file, "r", encoding="utf8") as reader, \
            ...
            for line in reader.read().strip().replace("\r", "").split("\n\n"):
                ...
                for word_tag in line:
                    ...
                    words.append(word)
                    tags.append(tag)
                example = [self._num_letter_normalize(word) for word in words]
